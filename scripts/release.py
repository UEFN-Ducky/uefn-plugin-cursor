#!/usr/bin/env python3
"""Zip + publish Cursor gateway to the UEFN Ducky Store via uds_release.
...
Usage:
  ...
  py scripts/release.py --publish --changelog "v1: Cursor API key + coding agent"
"""
...
def publish(zip_path: Path, *, category: str, changelog: str) -> None:
    ...
    release_args: dict = {
        "category": category,
        "changelog": changelog or "v1: Cursor API key + coding agent for Settings → LLMs",
        "publish": True,
        "categories": ["plugins", "gateways"],
        "tags": ["cursor", "llm", "gateway", "api-key", "coding-agent"],
        "name": "Cursor",
        "description": (
            "Cursor API key and Cursor coding agent. After Install + Enable, Cursor appears under "
            "Settings → LLMs → Providers & Keys and under Coding Agents."
        ),
    }
    if file_id:
